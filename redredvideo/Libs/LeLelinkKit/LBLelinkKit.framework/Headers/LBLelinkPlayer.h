...
/**
 ...
 @param player 当前播放器
 @param error 倍速播放状态，error 为 nil 时代表着设置倍速成功
 */
...
#pragma mark - 以下区间是推送媒体列表相关接口（包含视频列表和图片列表）

/// 是否能推送媒体列表，连接的收端能力判断
/// @param mediaType 媒体类型，现可支持视频LBLelinkMediaTypeVideoOnline/Local和照片LBLelinkMediaTypePhotoOnline/Local
- (BOOL)canPushMediaListType:(LBLelinkMediaType)mediaType;

/// 推送媒体列表
/// @param itemArray 媒体item数组
/// @param period 片源有效期时长，单位秒，必填，0代表长期有效
/// @param mediaId 开始播放媒体的媒体ID，非必填，默认从首个片源开始播放
/// @param mediaType 媒体类型，现可支持视频LBLelinkMediaTypeVideoOnline/Local和照片LBLelinkMediaTypePhotoOnline/Local，媒体方式只支持url
- (void)pushMediaList:(NSArray <LBLelinkPlayerItem *>*)itemArray validPeriod:(NSInteger)period startMediaId:(NSString *_Nullable)mediaId mediaType:(LBLelinkMediaType)mediaType;


/// 追加媒体列表
/// 追加的媒体类型与推送的媒体类型需保持一致，否则会有异常
/// @param itemArray 媒体item数组
/// @param period 片源有效期时长，单位秒，必填，0代表长期有效
- (void)appendMediaList:(NSArray <LBLelinkPlayerItem *>*)itemArray validPeriod:(NSInteger)period;
...
/// 指定播放媒体列表中某一集
/// @param mediaId 片源ID
- (void)playMediaListToMediaId:(NSString *)mediaId;


/// 播放下一集
- (void)playNextMediaInMediaList;

/// 播放上一集
- (void)playPrevMediaInMediaList;
