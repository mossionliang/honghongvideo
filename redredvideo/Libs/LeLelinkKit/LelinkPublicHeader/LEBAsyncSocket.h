...
@class LEBAsyncReadPacket;
@class LEBAsyncWritePacket;
@class LEBAsyncSocketPreBuffer;
@protocol LEBAsyncSocketDelegate;

NS_ASSUME_NONNULL_BEGIN

extern NSString *const LEBAsyncSocketException;
extern NSString *const LEBAsyncSocketErrorDomain;

extern NSString *const LEBAsyncSocketQueueName;
extern NSString *const LEBAsyncSocketThreadName;

extern NSString *const LEBAsyncSocketManuallyEvaluateTrust;
#if TARGET_OS_IPHONE
extern NSString *const LEBAsyncSocketUseCFStreamForTLS;
#endif
#define LEBAsyncSocketSSLPeerName     (NSString *)kCFStreamSSLPeerName
#define LEBAsyncSocketSSLCertificates (NSString *)kCFStreamSSLCertificates
#define LEBAsyncSocketSSLIsServer     (NSString *)kCFStreamSSLIsServer
extern NSString *const LEBAsyncSocketSSLPeerID;
extern NSString *const LEBAsyncSocketSSLProtocolVersionMin;
extern NSString *const LEBAsyncSocketSSLProtocolVersionMax;
extern NSString *const LEBAsyncSocketSSLSessionOptionFalseStart;
extern NSString *const LEBAsyncSocketSSLSessionOptionSendOneByteRecord;
extern NSString *const LEBAsyncSocketSSLCipherSuites;
#if !TARGET_OS_IPHONE
extern NSString *const LEBAsyncSocketSSLDiffieHellmanParameters;
#endif

#define LEBAsyncSocketLoggingContext 65535


typedef NS_ENUM(NSInteger, LEBAsyncSocketError) {
	LEBAsyncSocketNoError = 0,           // Never used
	LEBAsyncSocketBadConfigError,        // Invalid configuration
	LEBAsyncSocketBadParamError,         // Invalid parameter was passed
	LEBAsyncSocketConnectTimeoutError,   // A connect operation timed out
	LEBAsyncSocketReadTimeoutError,      // A read operation timed out
	LEBAsyncSocketWriteTimeoutError,     // A write operation timed out
	LEBAsyncSocketReadMaxedOutError,     // Reached set maxLength without completing
	LEBAsyncSocketClosedError,           // The remote peer closed the connection
	LEBAsyncSocketOtherError,            // Description provided in userInfo
};
...
/**
 * LEBAsyncSocket uses the standard delegate paradigm,
 ...
 * If you pass NULL, LEBAsyncSocket will automatically create it's own socket queue.
...
- (instancetype)initWithDelegate:(nullable id<LEBAsyncSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq;
- (instancetype)initWithDelegate:(nullable id<LEBAsyncSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq socketQueue:(nullable dispatch_queue_t)sq;

/**
 * Create LEBAsyncSocket from already connect BSD socket file descriptor
**/
...
+ (nullable instancetype)socketFromConnectedSocketFD:(int)socketFD delegate:(nullable id<LEBAsyncSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq error:(NSError**)error;

+ (nullable instancetype)socketFromConnectedSocketFD:(int)socketFD delegate:(nullable id<LEBAsyncSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq socketQueue:(nullable dispatch_queue_t)sq error:(NSError **)error;

#pragma mark Configuration

@property (atomic, weak, readwrite, nullable) id<LEBAsyncSocketDelegate> delegate;
...
- (void)getDelegate:(id<LEBAsyncSocketDelegate> __nullable * __nullable)delegatePtr delegateQueue:(dispatch_queue_t __nullable * __nullable)delegateQueuePtr;
- (void)setDelegate:(nullable id<LEBAsyncSocketDelegate>)delegate delegateQueue:(nullable dispatch_queue_t)delegateQueue;
...
**/
- (void)synchronouslySetDelegate:(nullable id<LEBAsyncSocketDelegate>)delegate;
- (void)synchronouslySetDelegateQueue:(nullable dispatch_queue_t)delegateQueue;
- (void)synchronouslySetDelegate:(nullable id<LEBAsyncSocketDelegate>)delegate delegateQueue:(nullable dispatch_queue_t)delegateQueue;

/**
 * By default, both IPv4 and IPv6 are enabled.
 * 
 * For accepting incoming connections, this means LEBAsyncSocket automatically supports both protocols,
 * and can simulataneously accept incoming connections on either protocol.
 * 
 * For outgoing connections, this means LEBAsyncSocket can connect to remote hosts running either protocol.
 * If a DNS lookup returns only IPv4 results, LEBAsyncSocket will automatically use IPv4.
 * If a DNS lookup returns only IPv6 results, LEBAsyncSocket will automatically use IPv6.
 ...
 * When a connection is accepted, a new instance of LEBAsyncSocket will be spawned to handle it,
 ...
 * 
 * Please note the recommended way of releasing a LEBAsyncSocket instance (e.g. in a dealloc method)
 ...
 * it is treated similarly to a timeout - the socket is closed with a LEBAsyncSocketReadMaxedOutError.
 ...
 * This is due to the fact that LEBAsyncSocket does NOT copy the data. It simply retains it.
 ...
 * 
 * - LEBAsyncSocketManuallyEvaluateTrust
 ...
 *     So LEBAsyncSocket will invoke the delegate method socket:shouldTrustPeer: passing the SecTrustRef.
 *
 ...
 *
 * - LEBAsyncSocketUseCFStreamForTLS (iOS only)
 *     The value must be of type NSNumber, encapsulating a BOOL value.
 *     By default LEBAsyncSocket will use the SecureTransport layer to perform encryption.
 ...
 *     
 *     However, if you absolutely must, you can instruct LEBAsyncSocket to use the old-fashioned encryption
 *     technique by going through the CFStream instead. So instead of using SecureTransport, LEBAsyncSocket
 ...
 *     
 *     Thus all the other keys in the given dictionary will be ignored by LEBAsyncSocket,
 ...
 *
 * - LEBAsyncSocketSSLProtocolVersionMin
 * - LEBAsyncSocketSSLProtocolVersionMax
 ...
 *
 * - LEBAsyncSocketSSLDiffieHellmanParameters (Mac OS X only)
 *     The value must be of type NSData.
 ...
 *     You MUST use manual trust evaluation instead (see LEBAsyncSocketManuallyEvaluateTrust).
 ...
 *     You MUST use LEBAsyncSocketSSLProtocolVersionMin & LEBAsyncSocketSSLProtocolVersionMin instead.
...
/**
 * LEBAsyncSocket maintains thread safety by using an internal serial dispatch_queue.
 ...
 * This is because the LEB API offers no mechanism to discover a queue's targetQueue.
 ...
 * - (dispatch_queue_t)newSocketQueueForConnectionFromAddress:(NSData *)address onSocket:(LEBAsyncSocket *)sock
 * {
 ...
 *     return socketQueue;
 * }
 * - (void)socket:(LEBAsyncSocket *)sock didAcceptNewSocket:(LEBAsyncSocket *)newSocket
 * {
 ...
 *   The faster and more powerful LEB API isn't notified properly in this case.
 * 
 ...
 * Example usage:
 * 
 * - (void)socket:(LEBAsyncSocket *)sock didConnectToHost:(NSString *)host port:(uint16_t)port
 * {
...
**/
- (nullable dispatch_queue_t)newSocketQueueForConnectionFromAddress:(NSData *)address onSocket:(LEBAsyncSocket *)sock;
 ...
 * The host parameter will be an IP address, not a DNS name.
 **/
- (void)socket:(LEBAsyncSocket *)sock didConnectToUrl:(NSURL *)url;

/**
 * Called when a socket has completed reading the requested data into memory.
 * Not called if there is an error.
**/
- (void)socket:(LEBAsyncSocket *)sock didReadData:(NSData *)data withTag:(long)tag;
...
**/
- (void)socket:(LEBAsyncSocket *)sock didReadPartialDataOfLength:(NSUInteger)partialLength tag:(long)tag;

/**
 * Called when a socket has completed writing the requested data. Not called if there is an error.
**/
- (void)socket:(LEBAsyncSocket *)sock didWriteDataWithTag:(long)tag;

/**
 * Called when a socket has written some data, but has not yet completed the entire write.
 * It may be used to for things such as updating progress bars.
**/
- (void)socket:(LEBAsyncSocket *)sock didWritePartialDataOfLength:(NSUInteger)partialLength tag:(long)tag;
...
**/
- (NSTimeInterval)socket:(LEBAsyncSocket *)sock shouldTimeoutReadWithTag:(long)tag
...
**/
- (NSTimeInterval)socket:(LEBAsyncSocket *)sock shouldTimeoutWriteWithTag:(long)tag
                                                                  elapsed:(NSTimeInterval)elapsed
                                                                bytesDone:(NSUInteger)length;
...
**/
- (void)socketDidCloseReadStream:(LEBAsyncSocket *)sock;
 ...
 * 
 * Note: If the LEBAsyncSocket instance is deallocated while it is still connected,
...
**/
- (void)socketDidDisconnect:(LEBAsyncSocket *)sock withError:(nullable NSError *)err;
...
**/
- (void)socket:(LEBAsyncSocket *)sock didReceiveTrust:(SecTrustRef)trust
